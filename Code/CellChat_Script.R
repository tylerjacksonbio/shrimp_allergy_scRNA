...
#############################################  Import required libraries and set directory paths #############################################  
suppressMessages({
  library(Seurat)
  library(CellChat)
  library(tidyverse)
...
cellchat_save_dir <- file.path(data_dir, "Cellchat_Analysis_Datasets_Only_LR_Pairs")

# Load dataset
PBMC_filtered_harmony <- readRDS(file.path(data_dir, "PBMC_DatasetMerged_adata.rds"))

# Testing
PBMC_filtered_harmony <- readRDS(file.path(base_dir, "PBMC_Dataset/Final_Datasets/Final_Annotated_Data/Final_Data_Upload_Analysis/Merged_adata.rds"))

Idents(PBMC_filtered_harmony) <- "Final_annotation_broad"

############################################# Running CellChat individually for each dataset to compare between them #############################################
...
prepare_cellchat_data <- function(object, condition) {
  ...
  meta <- data.frame(group = Idents(subset_obj), 
                     samples = condition,
                     row.names = names(Idents(subset_obj)))
...
create_and_save_cellchat <- function(condition) {
  ...
  saveRDS(cellchat, file = file.path(cellchat_save_dir, paste0("cellchat_", condition, "_data.rds")))
  return(cellchat)
}
...
walk2(processed_cellchat_objects, conditions, ~ saveRDS(.x, file.path(cellchat_save_dir, paste0("cellchat_", .y, "_processed.rds"))))

############################################# Running the CellChat comparison pipeline #############################################
# Define the comparisons
compare_conditions <- list(
  HCstim_vs_HCunstim = list(HC = processed_cellchat_objects[[1]], SA = processed_cellchat_objects[[2]]),
  SAstim_vs_SAunstim = list(HC = processed_cellchat_objects[[3]], SA = processed_cellchat_objects[[4]]),
  SAunstim_vs_HCunstim = list(HC = processed_cellchat_objects[[1]], SA = processed_cellchat_objects[[3]]),
  SAstim_vs_HCstim = list(HC_stim = processed_cellchat_objects[[2]], SA_stim = processed_cellchat_objects[[4]])
)
...
perform_comparison <- function(comparison_list, comparison_name) {
  ...
  saveRDS(cellchat, file = file.path(cellchat_save_dir, paste0("cellchat_comparison_", comparison_name, ".rds")))
}

walk2(compare_conditions, names(compare_conditions), perform_comparison)


# Define comparison names and their associated objects
comparisons <- c("HCstim_vs_HCunstim", "SAstim_vs_SAunstim", 
                 "SAstim_vs_HCstim", "SAunstim_vs_HCunstim")

# Load the merged CellChat objects
cellchat_objects <- lapply(comparisons, function(comp) {
  readRDS(file.path(cellchat_save_dir, paste0("cellchat_comparison_", comp, ".rds")))
})
names(cellchat_objects) <- comparisons  # Assign names for easier reference

# Create directories for storing final plots
lapply(comparisons, function(comp) {
  dir.create(file.path(plot_dir, comp, "Final_Plots"), showWarnings = FALSE, recursive = TRUE)
})

# Define signaling pathways of interest for each comparison
signaling_pathways <- list(
  "HCstim_vs_HCunstim" = c("MHC-II", "MHC-I"),
  "SAstim_vs_SAunstim" = c("MHC-II", "CLEC", "SELPLG", "SPP1"),
  "SAunstim_vs_HCunstim" = c("PARs", "TGFb", "MHC-II"),
  "SAstim_vs_HCstim" = c("TGFb", "BTLA", "TNF", "CLEC")
)

# Define gd T-cell signaling pathways
...
for (comp in comparisons) {
  cellchat <- cellchat_objects[[comp]]  # Get the corresponding merged cellchat object
  comp_dir <- file.path(plot_dir, comp)
  setwd(comp_dir)
  ...
  pdf(file.path(comp_dir, "Differential_interaction_web.pdf"))
  ...
  # Save the final CellChat object for the comparison
  saveRDS(cellchat, file.path(cellchat_save_dir, paste0("cellchat_", comp, "_comparison.rds")))
}

# Plotting the violins of specific signaling genes
for (comp in comparisons) {
  final_plot_dir <- file.path(plot_dir, comp, "Final_Plots")
